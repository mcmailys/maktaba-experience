...
function CssSpine({ title }) {
  return (
    <div className="h-44 sm:h-64 w-[48px] sm:w-[64px] rounded-[3px] bg-gradient-to-b from-[#1B1E26] via-[#13151A] to-[#0D0F13] border-x border-t border-[#D4AF37]/25 flex flex-col items-center justify-between py-3 shadow-[0_14px_28px_rgba(0,0,0,0.65)]">
      <span className="h-px w-3/4 bg-[#D4AF37]/60" />
      <span className="[writing-mode:vertical-rl] font-display text-sm sm:text-lg leading-none text-[#D4AF37] tracking-wide overflow-hidden max-h-[72%]">
        {title}
      </span>
      <span className="h-px w-3/4 bg-[#D4AF37]/60" />
    </div>
                    ...
                    >
                      {b.spine ? (
                        <img
                          ...
                          alt={`Tranche — ${b.title}`}
                          draggable={false}
                          className="h-44 sm:h-64 w-auto select-none shadow-[0_14px_28px_rgba(0,0,0,0.65)]"
                        />
                      ) : (
